...
public class BlankFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        hasil = (TextView)v.findViewById(R.id.hasil);
        subm = (Button)v.findViewById(R.id.set);
        subm.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                if (plaint.getText().toString().length()==0 || key.getText().toString().length()==0){
                    plaint.setError("Isi field");
                }else{
                    String text = new String();
                    text = plaint.getText().toString();
                    int kun = Integer.parseInt(key.getText().toString());
                    hasil.setText(enckript(text,kun));
                }
            }
        });
        return v;
    }
    public static String enckript(String plaintext, int keyIn){
        String cipher="";
